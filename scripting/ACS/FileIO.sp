...
void WriteDefaultMapListToFile()
{
    ...
    for (int i = 0; i < sizeof(g_strDefaultMapListArray); i++)
    {
        ...
        for (int j = 0; j < MAP_LIST_COLUMN_COUNT; j++)
        {
            ...
            if (j == MAP_LIST_COLUMN_GAMEMODE && StrEqual(strCurrentGameMode, g_strDefaultMapListArray[i][j]) == false)
            {
            ...
            if (j != MAP_LIST_COLUMN_COUNT - 1 && (j == MAP_LIST_COLUMN_MAP_NAME_START && strlen(g_strDefaultMapListArray[i][MAP_LIST_COLUMN_MAP_NAME_END]) == 0) == false)
...
void SetupMapListArrayFromFile()
{
    ...
    while (IsEndOfFile(hFile) == false && ReadFileLine(hFile, strLine, sizeof(strLine)))
    {
        ...
        if (strlen(strLineItems[MAP_LIST_COLUMN_GAMEMODE]) <= 1 || strlen(strLineItems[MAP_LIST_COLUMN_MAP_DESCRIPTION]) <= 1 || strlen(strLineItems[MAP_LIST_COLUMN_MAP_NAME_START]) <= 1)
            continue;
